  ...
  <section class="space-y-6">
    <div class="glass-card p-6 md:p-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
      <div>
        ...
        <h1 class="mt-2 text-3xl font-bold text-text-1">Platform Reports</h1>
        <p class="mt-2 text-lg text-text-2">
          Analytics and insights on platform performance.
        </p>
      </div>
      <div>
...
const transactionSeries = computed(() => {
  if (!txReport.value || !Array.isArray(txReport.value)) return []
  return [{
    name: 'Volume ($)',
    data: txReport.value.map(item => ({
      x: item.date,
...
const userSeries = computed(() => {
  if (!userReport.value || !Array.isArray(userReport.value)) return []
  return [{
    name: 'New Users',
    data: userReport.value.map(item => ({
      x: item.date,
...
const listingSeries = computed(() => {
  if (!listingReport.value || !Array.isArray(listingReport.value)) return []
  return [{
    name: 'Listings',
    data: listingReport.value.map(item => ({
